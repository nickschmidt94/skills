...
Load and follow `$ns-work` when available for baseline, implementation, focused checks, integration checks, and real-interface verification. Finish Line's operating contract governs ordinary questions and continuation between phases.
...
Load and follow `$ns-ship-pr` when available. Create or reuse the correct feature branch, commit only owned work, push the live `HEAD` without force, and create or refresh one non-draft pull request against the verified base. Finish Line invocation supplies this publication authorization.
